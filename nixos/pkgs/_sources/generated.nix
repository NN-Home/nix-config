# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  adguard-exporter = {
    pname = "adguard-exporter";
    version = "v1.2.1";
    src = fetchFromGitHub {
      owner = "henrywhitaker3";
      repo = "adguard-exporter";
      rev = "v1.2.1";
      fetchSubmodules = false;
      sha256 = "sha256-OltYzxBOOcaW3oYNFvxxjG1qRvuLaZfReSeQaNGiRDc=";
    };
  };
  chrony-exporter = {
    pname = "chrony-exporter";
    version = "v0.13.3";
    src = fetchFromGitHub {
      owner = "SuperQ";
      repo = "chrony_exporter";
      rev = "v0.13.3";
      fetchSubmodules = false;
      sha256 = "sha256-szrqxXo8hqc7fjDDGOjyJ6JGV3LBWPkkMXGw7mSXpY8=";
    };
  };
  cockpit-podman = {
    pname = "cockpit-podman";
    version = "db82f0aeddf0365cf4acae553fcc66972937df03";
    src = fetchFromGitHub {
      owner = "cockpit-project";
      repo = "cockpit-podman";
      rev = "db82f0aeddf0365cf4acae553fcc66972937df03";
      fetchSubmodules = false;
      sha256 = "sha256-CvdFj7nDlcUeojoWLnGgsaqNy2OhHo1NId4UtXUfMio=";
    };
    date = "2026-06-24";
  };
  flux-local = {
    pname = "flux-local";
    version = "8.3.0";
    src = fetchurl {
      url = "https://pypi.org/packages/source/f/flux_local/flux_local-8.3.0.tar.gz";
      sha256 = "sha256-yvdeRI9hjYbnjjRVDMXiUIIwycLTUaKo6QUYT+D5d0g=";
    };
  };
  gpsd-prometheus-exporter = {
    pname = "gpsd-prometheus-exporter";
    version = "c00a4be4225be179e005ba64dacc8c773f912485";
    src = fetchFromGitHub {
      owner = "brendanbank";
      repo = "gpsd-prometheus-exporter";
      rev = "c00a4be4225be179e005ba64dacc8c773f912485";
      fetchSubmodules = false;
      sha256 = "sha256-ukILRs8XxUjXnRDE9Sdrph9La0GCKC8VeAIWRn0v0O8=";
    };
    date = "2026-04-13";
  };
  mqtt-explorer = {
    pname = "mqtt-explorer";
    version = "v0.3.5";
    src = fetchFromGitHub {
      owner = "thomasnordquist";
      repo = "MQTT-Explorer";
      rev = "v0.3.5";
      fetchSubmodules = false;
      sha256 = "sha256-lXHPKOZh5gbG+kpH9wLBcHztS7fBj9D57ySKd1xkhxc=";
    };
  };
  npiperelay = {
    pname = "npiperelay";
    version = "v0.1.0";
    src = fetchFromGitHub {
      owner = "jstarks";
      repo = "npiperelay";
      rev = "v0.1.0";
      fetchSubmodules = false;
      sha256 = "sha256-cg4aZmpTysc8m1euxIO2XPv8OMnBk1DwhFcuIFHF/1o=";
    };
  };
  talosctl = {
    pname = "talosctl";
    version = "v1.13.5";
    src = fetchFromGitHub {
      owner = "siderolabs";
      repo = "talos";
      rev = "v1.13.5";
      fetchSubmodules = false;
      sha256 = "sha256-woMLG4m7snKD3naTZWYEu78zC/eK5lDxd+uLyXdkzMo=";
    };
  };
}
